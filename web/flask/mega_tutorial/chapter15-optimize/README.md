...
这是`Flask Mega-Tutorial`系列的第十五部分，我将使用适用于大型应用的风格重构本应用。

`Microblog`已经是一个初具规模的应用了，所以我认为这是讨论`Flask`应用如何在持续增长中不会变得混乱和难以管理的好时机。 Flask是一个框架，旨在让你选择以任何方式来组织项目，基于该理念，在应用日益庞大或者技能水平变化的时候，才有可能更改和调整其结构。
